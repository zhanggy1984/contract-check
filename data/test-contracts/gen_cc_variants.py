# -*- coding: utf-8 -*-
"""7.8 薄弱点①排查：single_party 规则泛化性验证用变体合同生成。

生成 5 个 PDF（正文统一、仅签署区形态不同），覆盖单方签署 3 种缺陷形态 + good 对照：
  cc_gen_good.pdf        双方均实际签署盖章           → 期望 0 违规（无误报）
  cc_gen_v0_underline    乙方下划线空白占位（b4 形态）→ 期望检出 single_party
  cc_gen_v1_empty        乙方盖章行冒号后为空（无下划线）→ 期望检出
  cc_gen_v2_missing      缺整个乙方签署区            → 期望检出
  cc_gen_v3_mixed        甲方既有署名又有空白占位（方向对称）→ 期望检出

只改签署区，正文完全一致，隔离「规则对签署形态的判定」这一变量。
"""
import os
...
# 签署区变体：key=(文件名, 签署区行列表)
SIGNATURES = {
    "cc_gen_good": [
        "甲方（盖章）：北京云途科技有限公司        法定代表人或授权代表（签字）：王强",
        "乙方（盖章）：上海品控贸易有限公司        法定代表人或授权代表（签字）：李敏",
        "签订日期：2026 年 6 月 1 日",
    ],
    "cc_gen_v0_underline": [  # b4 原形态：乙方既有署名又有下划线空白占位
        "甲方（盖章）：北京云途科技有限公司        乙方（盖章）：上海品控贸易有限公司",
        "签订日期：2026 年 6 月 1 日",
        "乙方（盖章）：__________",
    ],
    "cc_gen_v1_empty": [  # 乙方盖章行冒号后为空、无下划线（未填写主体）
        "甲方（盖章）：北京云途科技有限公司        法定代表人或授权代表（签字）：王强",
        "乙方（盖章）：",
        "签订日期：2026 年 6 月 1 日",
    ],
    "cc_gen_v2_missing": [  # 缺整个乙方签署区
        "甲方（盖章）：北京云途科技有限公司        法定代表人或授权代表（签字）：王强",
        "签订日期：2026 年 6 月 1 日",
    ],
    "cc_gen_v3_mixed": [  # 甲方混合：甲方既有署名又有空白占位（方向对称验证）
        "甲方（盖章）：北京云途科技有限公司",
        "甲方（盖章）：____________",
        "乙方（盖章）：上海品控贸易有限公司        法定代表人或授权代表（签字）：李敏",
        "签订日期：2026 年 6 月 1 日",
...
def build(fname: str, sig_lines: list[str]) -> None:
    ...
    story = []
    story.append(Paragraph(BODY[0], STYLE_TITLE))
    for line in BODY[1:]:
