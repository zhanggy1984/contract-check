...
**观测埋点**
- 校验流水线（LangGraph 状态图）每个节点、每次 LLM 调用落结构化事件：`trace_id` / `seq` / `interface` / `status` / `error_type` / `duration_ms` / `usage`；
- 埋点走旁路、不阻断业务；**抽取阶段的多任务并发**尤其依赖可靠的 span 边界，才能还原真实调用重叠关系。
...
- **为什么单测没抓住**——原测试替身的临界区为零，客观上不存在争用；
  **"测试全绿"只能证明写下的断言成立，不能证明争用没有发生**；
- **修复**：每个派生任务从父上下文 `copy()` 出独立上下文，隔离写入；
- **复验**：修复后并发批次全部正常，且并发 span 的重叠关系在观测面上可逐条还原。
