...
| **规则管理** | 本体规则自动维护（规则名自动转大白话法律术语，列表只展示当前本体版本规则、按 ID 倒序）；新建规则仅支持**语义 LLM** 类型（确定性规则由本体生成，用户不手写 SPARQL），rule_iri 由规则名自动生成（冲突自动加后缀）；支持编辑 / 启停 / dry-run 试跑 / 删除（被历史校验记录引用的规则禁止删除，改用「失效」），语义规则支持 any / all 聚合粒度 |
